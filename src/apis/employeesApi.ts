...
export const employeesApi = {
  ...
  async create(payload: EmployeeCreate): Promise<Employee> {
    const res = await apiClient.post<Employee>(API_ROUTES.employees.list, payload);
    return res.data;
  },
  ...
  async update(id: number | string, payload: EmployeeUpdate): Promise<Employee> {
    const res = await apiClient.put<Employee>(API_ROUTES.employees.byId(id), payload);
    return res.data;
  },
};
